...
Expression : [["le bonheur", "la bonté", "les choses magiques", "la plus grande gloire", "les femmes"],
			 ...
			 ["la clé de la vie", "l'élévation de l'âme", "que nous soyons assez intelligents pour les recevoir", "de ne jamais tomber", "le dernier amour d'un homme"]]
};
...
let DebutCitation = array[0][Math.floor(Math.random()*i)];
let MilieuCitation = array[1][Math.floor(Math.random()*i)];
let FinCitation = array[2][Math.floor(Math.random()*i)];

let CompleteCitation = `${DebutCitation} ${MilieuCitation} ${FinCitation}`;
